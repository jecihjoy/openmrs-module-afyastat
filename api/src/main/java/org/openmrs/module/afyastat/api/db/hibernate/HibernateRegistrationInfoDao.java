...
/**
 * It is a default implementation of {@link org.openmrs.module.afyastat.api.db.RegistrationInfoDao}.
 */
public class HibernateRegistrationInfoDao extends HibernateSingleClassInfoDao<RegistrationInfo> implements RegistrationInfoDao {
	 ...
	 */
	@Override
	public RegistrationInfo getRegistrationDataById(final Integer id) {
		return getById(id);
	}
	 ...
	 */
	@Override
	public RegistrationInfo getRegistrationDataByUuid(final String uuid) {
		Criteria criteria = getSessionFactory().getCurrentSession().createCriteria(mappedClass);
		criteria.add(Restrictions.eq("uuid", uuid));
		criteria.add(Restrictions.eq("voided", Boolean.FALSE));
		return (RegistrationInfo) criteria.uniqueResult();
	}
	
	/**
	 * Get registration data based on the temporary uuid assigned to a patient created through the
	 * registration form and / or the real uuid of the patient data created after processing the
	 * registration form (for new patient) or the real uuid of the existing patient (for existing
	 * patient).
	 * 
	 * @param temporaryUuid the temporary uuid assigned to a patient.
	 * @param assignedUuid the real uuid of a newly created patient or the real uuid of an existing
	 *            patient.
	 * @return the registration data based on the temporary uuid and / or the assigned uuid.
	 */
	@Override
	@SuppressWarnings("unchecked")
	public List<RegistrationInfo> getRegistrationData(final String temporaryUuid, final String assignedUuid) {
		Criteria criteria = getSessionFactory().getCurrentSession().createCriteria(mappedClass);
	...
	@Override
	@Transactional
	public RegistrationInfo saveRegistrationData(final RegistrationInfo registrationData) {
	...
	@Override
	@Transactional
	public void deleteRegistrationData(final RegistrationInfo registrationData) {
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<RegistrationInfo> getRegistrationData(final Integer pageNumber, final Integer pageSize) {
		Criteria criteria = getSessionFactory().getCurrentSession().createCriteria(mappedClass);
		if (pageNumber != null) {
